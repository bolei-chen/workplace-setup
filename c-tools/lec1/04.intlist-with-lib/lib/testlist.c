...
int main( void )
{
	...
	testcond( head == 100, "head([100]) == 100" );
	testcond( intlist_kind( tail ) == intlist_is_nil,
		"kind(tail([100])) == nil" );

	l = intlist_cons( 200, l );
	get_intlist_cons( l, &head, &tail );
	testcond( head == 200, "head([200,100]) == 200" );
	get_intlist_cons( tail, &head, &tail );
	testcond( head == 100, "head(tail([200,100])) == 100" );
	...
	testcond( strcmp( result, "[ 400,300,200,100 ]") == 0, "sprint test" );

	int sum = 0;
	foreach_intlist( sumcb, (void *)&sum, l );
	testcond( sum == 1000, "sum 1000" );
